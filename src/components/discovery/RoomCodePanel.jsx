...
export function RoomCodePanel({ roomCode, peerId, shareLink }) {
      ...
      <div className="grid gap-3 text-sm text-[color:var(--text-muted)]">
        <div className="flex items-start justify-between gap-3 rounded-2xl border border-white/10 bg-white/5 px-3 py-2">
          <div className="min-w-0 flex-1">
            <div className="text-xs uppercase tracking-[0.2em]">{t("radar.peerId")}</div>
            <div className="break-all text-[color:var(--text-primary)]">{peerId}</div>
          </div>
          <button
            type="button"
          ...
          >
            {copiedField === "peerId" ? <span className="text-[10px]">{t("actions.copied")}</span> : <Copy size={16} />}
          </button>
        </div>
